...
function generateJSONData(numUsers: number = 50) {
    const accounts = [];
    const userInfos = [];
    ...
    for (let i = 0; i < numUsers; i++) {
        ...
        // Generate a fake ObjectId (24 character hex string)
        const accountId = generateRandomString(24);
...
async function main() {
    ...
    if (isNaN(numUsers) || numUsers <= 0) {
        console.error('Please provide a valid number of users to generate');
        process.exit(1);
    }

    console.log(`Generating ${numUsers} fake users...`);

    const data = generateJSONData(numUsers);
    ...
    // Write user infos data
    fs.writeFileSync(
        path.join(dataDir, 'userinfos.json'),
        JSON.stringify(data.userInfos, null, 2)
    );

    console.log(`Successfully generated ${numUsers} fake users!`);
    console.log('Files created:');
    console.log('- data/accounts.json');
    console.log('- data/userinfos.json');

    // Print some statistics
    const maleCount = data.userInfos.filter(u => u.gender === 'male').length;
    const femaleCount = data.userInfos.filter(u => u.gender === 'female').length;
    const otherCount = data.userInfos.filter(u => u.gender === 'other').length;

    console.log(`Gender distribution: ${maleCount} male, ${femaleCount} female, ${otherCount} other`);
}
